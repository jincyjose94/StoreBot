		...
		else
		{
			if(ultraf<=front_obst)
			{
				...
				while((ultraf>=front_obst)&&(ultra_rl<=right_obst))
				{
					...
					if((ultraf<=front_obst)||(ultra_rl>=right_obst))  //extra added here
					break;
					else continue;
				}
				if((ultraf<=front_obst)||(ultra_rl>=right_obst))
				{
					...
					if(ultra_rl<=right_obst)
						{
						print_string("continue_executed");
					...
					else
					{
						...
						while((ultraf>=front_obst)&&(ultra_rl<=right_obst))
						{
							//print_string("inside_while_loop_1");
							front();
							ultraf=ultrasonic_front();
							ultra_rl=ultra_leftright_check(not_r_l);
						...
						if(ultra_rl>=right_obst)
						{
							...
							ultraf=ultrasonic_front();
							while(ultraf>=front_obst)
							{
								front();
								print_string("while2");
								ultraf=ultrasonic_front();
								if(ultraf<=front_obst)  //extra 3 lines
								break;
								else continue;
							}
							if(ultraf>=front_obst)
				...
				if((ultra_rl<=right_obst)&&(ultraf<=front_obst))
				{
